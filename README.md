...
.env 文件：用于本地测试和存放非敏感配置（如关键词）。
GitHub Secrets：用于云端安全运行，存放敏感信息（如邮箱密码）。
...
SEARCH_KEYWORDS=Rydberg atom, Rydberg state
...
.env 仅用于本地：此文件切勿上传到GitHub。在云端运行时，邮箱信息需通过下一步的 Secrets 配置。
...
在仓库中打开文件：.github/workflows/arxiv_daily.yml
找到 schedule 部分，修改 cron 表达式的值。
...
保存并提交文件更改。提交后，GitHub Actions 将按新时间表运行。
🚀 开始使用

方法一：一键 Fork 并配置（推荐）

Fork 本仓库到你自己的 GitHub 账户。
在你 Fork 出的仓库中，按上方 第二步：配置 GitHub Secrets 完成邮箱密钥设置。
程序将在你设定的时间自动运行，可在 Actions 标签页查看运行状态。
方法二：手动运行测试
...
GitHub Secrets 中的邮箱和授权码（16位SMTP码）是否正确。
仓库 Actions 页面最近一次运行日志是否有报错（红色提示）。
检查邮箱的垃圾邮件文件夹。
Q：定时任务到点没有运行？
A：

确认 .github/workflows/arxiv_daily.yml 文件已成功提交。
在 Actions 页面，点击工作流名称，查看右侧是否显示 This workflow has a schedule event trigger... 及下次运行时间。
GitHub Actions 的定时触发可能有几分钟延迟。
Q：如何修改搜索的关键词？
